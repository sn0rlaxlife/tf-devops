...
module "network" {
  ...
  resource_group_name = azurerm_resource_group.azucar1.name
  address_space       = "10.52.0.0/16"
  subnet_prefixes     = ["10.52.0.0/24"]
  subnet_names        = ["subnet1"]
  depends_on          = [azurerm_resource_group.azucar1]
}
...
module "aks_ssh-key" {
  source  ="Azure/aks/azurerm/modules/ssh-key"
  version ="4.16.0"
}
